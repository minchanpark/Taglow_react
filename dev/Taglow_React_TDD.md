...
| Styling | view page-local plain CSS | MVC 구조 유지, 전역 CSS 제거, 화면별 책임 명확화 |
...
- runtime participant API는 실 서버 gateway/mapper 기반 구현을 사용한다.
...
| `lib/theme/app_colors.dart` | 각 `src/view/*/*.css` page-local color token |
...
Flutter의 `participantServiceProvider`는 React에서 실 서버 gateway/mapper singleton 조립으로 옮긴다.
...
전역 theme 레이어는 두지 않는다. Flutter theme 기준에서 필요한 색상, spacing, typography, frame 규칙은 각 page CSS의 root class에 가까이 둔다.

### 9-1. Page-local shell

각 page CSS의 root class가 아래를 함께 책임진다.

- mobile frame width, desktop centered frame, safe-area padding
- font family, font smoothing, text rendering
- page-local color custom properties
- `box-sizing`, link reset, button/input font reset
- page-specific layout, interaction state, loading/error/success visuals

### 9-2. Guardrails

- `src/theme` 폴더와 runtime global CSS를 만들지 않는다.
- 여러 page에서 값이 비슷해도 먼저 page-local CSS로 명시한다.
- 공통화는 실제 중복이 유지보수 문제로 확인될 때 view/component 경계 안에서만 검토한다.
- 하단 입력바, tag sticker, thanks/final button 같은 UI 규칙은 해당 view 또는 component CSS 옆에 둔다.
